...
public abstract class AbstractBasicMessageTransponder extends AbstractMessageTransponder {
	...
	protected void distributeEvent(Message message) {
		System.out.printf(">>AbstractBasicMessageTransponder distributeEvent \n" );
		if( message == null){
			System.out.println("message is null");
		}
		if( message.getEntries() == null){
			System.out.println(" message.entries is null");
		}
		System.out.printf("size %s \n",Integer.toString( message.getEntries().size()));
		//获取操作实体
...
//			}
			...
			if (ignoreEntryTypes != null && ignoreEntryTypes.size() > 0
					&& ignoreEntryTypes.stream().anyMatch(t -> entry.getEntryType() == t)) {
				System.out.printf(">>ignore this entry  type: %s \n",entry.getEntryType().toString());
				continue;
			}
			//canal 改变信息
			...
			try {
				...
				System.out.println(">>parse rowChange  error");
				throw new CanalClientException("错误 ##转换错误 , 数据信息:" + entry.toString(),
						e);
			}
			System.out.println(">>doing distributeByAnnotation ");
	...
	protected void distributeByAnnotation(String destination,
	                                      ...
	                                      String tableName,
	                                      CanalEntry.RowChange rowChange) {
		System.out.printf(">>distributeByAnnotation");
		//对注解的监听器进行事件委托
